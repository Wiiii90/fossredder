...
void SpdlogDebugger::writeText(const std::string& relPath, const std::string& text) {
    ...
    // Avoid printing very large/poppler metadata content to console.
    // If the path indicates poppler metadata, forward to backend and log a short message only.
    try {
    ...
    // default behavior: log text and forward to backend
...
void SpdlogDebugger::writeBytes(const std::string& relPath, const std::vector<uint8_t>& data) {
    if (!m_enabled_) return;
    // For bytes we delegate to backend if provided, and log action
    if (m_backend_) {
        try {
