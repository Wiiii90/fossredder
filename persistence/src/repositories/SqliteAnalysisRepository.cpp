...
namespace {
constexpr auto kSelectAnalysis =
    "SELECT id, name, type, config_json, filter_spec, created_at, updated_at, schema_version FROM analyses";
...
std::shared_ptr<Analysis> readAnalysis(persistence::StmtGuard& s) {
    ...
    a->createdAt = s.columnText(5); a->updatedAt = s.columnText(6);
...
void SqliteAnalysisRepository::addAnalysis(const std::shared_ptr<Analysis>& analysis) {
    ...
    persistence::StmtGuard stmt(pimpl_->db->handle(),
        "INSERT OR IGNORE INTO analyses (id, name, type, config_json, filter_spec, created_at, updated_at, schema_version)"
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?);");
    if (!stmt) return;
    ...
    stmt.bindText(5, analysis->filterSpec); stmt.bindText(6, analysis->createdAt);
    stmt.bindText(7, analysis->updatedAt);  stmt.bindInt (8, analysis->schemaVersion);
    stmt.step();
}
...
std::vector<std::shared_ptr<Analysis>> SqliteAnalysisRepository::getAnalyses() const {
    ...
    persistence::StmtGuard stmt(pimpl_->db->handle(),
        std::string(kSelectAnalysis) + " ORDER BY id;");
    if (!stmt) return out;
...
std::optional<std::shared_ptr<Analysis>> SqliteAnalysisRepository::getAnalysisById(const std::string& id) const {
    persistence::StmtGuard stmt(pimpl_->db->handle(),
        std::string(kSelectAnalysis) + " WHERE id = ? LIMIT 1;");
...
void SqliteAnalysisRepository::updateAnalysis(const std::shared_ptr<Analysis>& analysis) {
    if (!analysis || analysis->id.empty()) return;
    persistence::StmtGuard stmt(pimpl_->db->handle(),
        "UPDATE analyses SET name=?, type=?, config_json=?, filter_spec=?, created_at=?, updated_at=?, schema_version=?"
        " WHERE id=?;");
    if (!stmt) return;
    ...
    stmt.bindText(5, analysis->createdAt);  stmt.bindText(6, analysis->updatedAt);
    stmt.bindInt (7, analysis->schemaVersion); stmt.bindText(8, analysis->id);
    stmt.step();
}
