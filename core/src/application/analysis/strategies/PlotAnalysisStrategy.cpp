...
ContractTypeIndex buildContractTypeIndex(const core::domain::AppState& state, bool normalize)
{
...
std::vector<std::shared_ptr<Transaction>> collectMatchedTransactions(const core::domain::AppState& state,
                                                                     const Filter& filter,
...
AnalysisResult PlotAnalysisStrategy::compute(const Analysis& analysis, const core::domain::AppState& state, const Filter& filter) const {
