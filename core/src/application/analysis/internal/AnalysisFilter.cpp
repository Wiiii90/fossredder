...
bool compareInt(AnalysisFilterOperator op, int lhs, int rhs)
{
    switch (op) {
    case AnalysisFilterOperator::GreaterEqual: return lhs >= rhs;
    case AnalysisFilterOperator::LessEqual: return lhs <= rhs;
    case AnalysisFilterOperator::Greater: return lhs > rhs;
    case AnalysisFilterOperator::Less: return lhs < rhs;
    case AnalysisFilterOperator::Equal: return lhs == rhs;
    case AnalysisFilterOperator::NotEqual: return lhs != rhs;
    }
    return false;
}

bool compareDouble(AnalysisFilterOperator op, double lhs, double rhs)
{
    switch (op) {
    case AnalysisFilterOperator::GreaterEqual: return lhs >= rhs;
    case AnalysisFilterOperator::LessEqual: return lhs <= rhs;
...
std::optional<AnalysisFilterClause> parseClause(const std::string& token)
{
    struct OperatorSpec {
    ...
    static constexpr OperatorSpec operators[] = {
        {core::constants::filters::operators::kGreaterEqual, AnalysisFilterOperator::GreaterEqual},
        {core::constants::filters::operators::kLessEqual, AnalysisFilterOperator::LessEqual},
        {core::constants::filters::operators::kNotEqual, AnalysisFilterOperator::NotEqual},
        {core::constants::filters::operators::kGreater, AnalysisFilterOperator::Greater},
        {core::constants::filters::operators::kLess, AnalysisFilterOperator::Less},
        {core::constants::filters::operators::kEqual, AnalysisFilterOperator::Equal},
    };
...
void addAmountPredicate(core::application::analysis::AnalysisFilter& filter, const AnalysisFilterClause& clause)
{
    double value = 0.0;
    try { value = std::stod(clause.value); } catch (...) { value = 0.0; }

    filter.addPredicate([value, op = clause.op](const std::shared_ptr<Transaction>& transaction, const WorkspaceCatalog&) {
        return transaction && compareDouble(op, transaction->amount(), value);
    });
}

void addContractTypePredicate(core::application::analysis::AnalysisFilter& filter, const AnalysisFilterClause& clause)
{
...
void addPropertyPredicate(core::application::analysis::AnalysisFilter& filter, const AnalysisFilterClause& clause)
{
...
void addAllocatablePredicate(core::application::analysis::AnalysisFilter& filter, const AnalysisFilterClause& clause)
{
    ...
    filter.addPredicate([expected](const std::shared_ptr<Transaction>& transaction, const WorkspaceCatalog&) {
        return transaction && transaction->isAllocatable() == expected;
    });
}
...
{
    std::vector<std::shared_ptr<core::domain::Transaction>> out;
