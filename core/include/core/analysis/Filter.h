...
class Filter {
public:
    using Pred = std::function<bool(const std::shared_ptr<Transaction>&, const AppState&)>;

    void addPredicate(Pred p) { preds_.push_back(std::move(p)); }

    bool matches(const std::shared_ptr<Transaction>& t, const AppState& state) const {
