...
class Filter {
public:
    using Pred = std::function<bool(const std::shared_ptr<core::domain::Transaction>&, const core::domain::WorkspaceState&)>;

    void addPredicate(Pred p) { preds_.push_back(std::move(p)); }

    bool matches(const std::shared_ptr<core::domain::Transaction>& t, const core::domain::WorkspaceState& state) const {
