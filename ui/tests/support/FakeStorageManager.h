/**
 * @file ui/tests/support/FakeStorageManager.h
 * @brief Test helper storage manager for UI source tests.
 */
...
public:
    void setAtomicStoreSave(AtomicStoreSave saveFn) override { saveFn_ = std::move(saveFn); }
    void setAtomicStoreLoad(AtomicStoreLoad loadFn) override { loadFn_ = std::move(loadFn); }
    void setDeletionImpactCallback(DeletionImpactCallback cb) override { deletionImpactCallback_ = std::move(cb); }

    std::optional<std::string> loadLatestPath() const override { return latestPath_; }
