...
void SessionState::loadFromState(
...
TransactionFilter *
SessionState::statementTransactions(const QString &statementId) {
  return filters_.statementTransactions(statementId, models_.transactions());
}

TransactionFilter *
SessionState::propertyTransactions(const QString &propertyId) {
...
void SessionState::applyDeletionImpact(const DeletionImpact &impact) {
  SessionMutationState::applyDeletionImpact(impact, models_, filters_);
  models_.refreshContractTypes();
  bumpDataRevision();
}

void SessionState::setTransactionPropertyIdsImmediate(
    const QString &txId, const QStringList &propertyIds) {
  SessionMutationState::setTransactionPropertyIdsImmediate(txId, propertyIds,
                                                           models_);
  bumpDataRevision();
}
...
QString
SessionState::normalizedStringListKey(const QVariantList &values) const {
  const QVariantList normalized =
      SessionMutationState::normalizeStrings(values);
  QStringList out;
...
QVariantList SessionState::addUniqueTrimmed(const QVariantList &values,
                                            const QString &value) const {
  return SessionMutationState::addUniqueTrimmed(values, value);
}

QVariantList SessionState::removeAt(const QVariantList &values,
                                    int index) const {
  return SessionMutationState::removeAt(values, index);
}

QVariantList SessionState::removeString(const QVariantList &values,
                                        const QString &value) const {
  return SessionMutationState::removeString(values, value);
}

bool SessionState::formStateChanged(const QString &savedName,
...
QVariantList SessionState::insertAt(const QVariantList &values, int index,
                                    const QVariant &value) const {
  return SessionMutationState::insertAt(values, index, value);
}

QVariantList
SessionState::pruneAndAppendMissing(const QVariantList &preferredIds,
                                    const QVariantList &availableIds) const {
...
int SessionState::indexOfKeyValue(const QVariantList &rows, const QString &key,
                                  const QVariant &value) const {
  return ui::indexOfKeyValue(rows, key, value);
}

int SessionState::indexOfString(const QVariantList &values,
                                const QString &value) const {
...
QString SessionState::navigatedSelectionId(const QVariantList &rows,
                                           const QString &currentId, int delta,
                                           int defaultIndex,
                                           const QString &idKey) const {
  return ui::navigatedSelectionId(rows, currentId, delta, defaultIndex, idKey);
}

QVariantList
SessionState::displayRowsWithEmpty(const QVariantList &rows,
                                   const QString &emptyDisplay,
                                   const QString &displayKey) const {
  return ui::displayRowsWithEmpty(rows, emptyDisplay, displayKey);
}

QVariantList SessionState::rowIds(const QVariantList &rows,
                                  const QString &idKey) const {
...
QVariantMap SessionState::mapWithKeyValue(const QVariantMap &base,
                                          const QString &key,
                                          const QVariant &value) const {
...
QVariantList
SessionState::normalizeTransactionDrafts(const QVariantList &values) const {
...
QVariantMap
SessionState::createDraftListState(const QVariantList &drafts, int currentIndex,
                                   const QVariantMap &emptyDraft) const {
  return SessionMutationState::createDraftListState(drafts, currentIndex,
                                                    emptyDraft);
}

QVariantMap
SessionState::insertDraftAfterCurrent(const QVariantList &drafts,
                                      int currentIndex,
                                      const QVariantMap &emptyDraft) const {
  return SessionMutationState::insertDraftAfterCurrent(drafts, currentIndex,
                                                       emptyDraft);
}
...
QVariantMap
SessionState::currentDraftState(const QVariantList &drafts, int currentIndex,
                                const QVariantMap &emptyDraft) const {
  return SessionMutationState::currentDraftState(drafts, currentIndex,
                                                 emptyDraft);
}
...
QVariantList SessionState::orderWithInsertedId(const QVariantList &currentOrder,
  ...
  return ui::orderWithInsertedId(currentOrder, availableIds, insertedId,
                                 insertAfterIndex);
}
...
QVariantMap SessionState::orderedSelectionState(
    const QVariantList &rows, const QVariantList &preferredOrder,
    int currentIndex, const QString &selectedId, const QString &idKey) const {
  return ui::orderedSelectionState(rows, preferredOrder, currentIndex,
                                   selectedId, idKey);
}
...
QVariantMap SessionState::navigateSelectionState(const QVariantList &rows,
                                                 ...
                                                 const QString &idKey) const {
  return ui::navigateSelectionState(rows, currentIndex, selectedId, delta,
                                    defaultIndex, idKey);
}

QVariantMap SessionState::deleteReselectionState(
    const QVariantList &rows, const QVariantList &preferredOrder,
    int currentIndex, const QString &removedId, const QString &idKey) const {
  return ui::deleteReselectionState(rows, preferredOrder, currentIndex,
                                    removedId, idKey);
}
...
QVariantMap
SessionState::basicFormState(const QString &name, const QVariantList &aliases,
